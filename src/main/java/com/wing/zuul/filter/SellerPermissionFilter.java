...
@Component
public class SellerPermissionFilter extends ZuulFilter {
    ...
    public boolean shouldFilter() {
        ...
        if("/orders/finish".equals(request.getRequestURI()) && HttpMethod.PUT.matches(request.getMethod())) {
            return true;
        }
        return false;
    }
    ...
    public Object run() {
        ...
        if(StringUtils.isBlank(token)) {
            ...
            if(value != null && (pos = value.indexOf("_")) != -1){
                if(!"SELLER".equals(value.substring(pos + 1))) {
                    ctx.setSendZuulResponse(false);
                    ctx.setResponseStatusCode(HttpStatus.SC_UNAUTHORIZED);
                }
            }else {
